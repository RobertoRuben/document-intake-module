...
import { useHamletTableContext } from "../../context/hamlet-table.context";
import { useHamletContext } from "../../context/hamlet.context";
...
interface HamletTableContextType {
    ...
    columns: ColumnDef<Hamlet, unknown>[];
    tableVariants: Variants;
    dataVersion: number;
    searchTerm: string;
    onSearchChange: (value: string) => void;
    handleBulkDelete: () => Promise<void>;
    getSelectedHamletIds?: () => number[];
    hamlets: Hamlet[];
    paginationMeta: PaginationMetaModel;
}

export const HamletTable: React.FC = () => {
    ...
    const {
        ...
        hamlets,
        paginationMeta
    } = useHamletTableContext() as unknown as HamletTableContextType;

    const { handleExportToExcel: exportToExcel, isLoading } = useHamletContext();

    const handleExportToExcel = async () => {
        if (getSelectedHamletIds) {
            ...
            if (selectedIds.length > 0) {
                try {
                    ...
                    toast.error("Error de exportación", {
                        description: "No se pudieron exportar los caseríos seleccionados."
                    });
                }
            } else {
                toast.warning("Selección vacía", {
                    description: "No hay caseríos seleccionados para exportar"
                });
            }
        }
    };
    ...
    const handleOpenBulkDeleteModal = () => {
        if (getSelectedHamletIds) {
            ...
            if (selectedIds.length > 0) {
                ...
                toast.warning("Selección vacía", {
                    description: "No hay caseríos seleccionados para eliminar"
                });
            }
        }
    };
    ...
    return (
        <div className="w-full">
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        <Table>
                            ...
                            <TableBody>
                                {isLoading && hamlets.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={columns.length} className="p-4 text-center">
                                            <div className="flex justify-center items-center">
                                                <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-gray-900"></div>
                                                <span className="ml-2">Cargando caseríos...</span>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={columns.length} className="h-24">
                                            <EmptyStateMessage 
                                                message="No se encontraron caseríos"
                                            />
                                        </TableCell>
                                    </TableRow>
                                )}
            ...
            <DeleteModal
                ...
                title="Eliminar caseríos seleccionados"
                description={`¿Estás seguro que deseas eliminar ${totalSelectedRows} ${
                    totalSelectedRows === 1 ? "caserío" : "caseríos"
