...
export const useDepartmentConnectionsContainerHook = () => {
  ...
  const [selectedDepartmentConnection, setSelectedDepartmentConnection] =
    useState<DepartmentConnection | undefined>(undefined);

  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [departmentConnectionToDelete, setDepartmentConnectionToDelete] =
    useState<DepartmentConnection | undefined>(undefined);
  const [selectedDepartmentConnectionIds, setSelectedDepartmentConnectionIds] =
    useState<number[]>([]);
  ...
  const fetchDepartmentConnections = useCallback(
    async (page = currentPage, search = searchTerm, forceRefresh = false) => {
      ...
      try {
        ...
        if (search) {
          response =
            await departmentConnectionService.searchDepartmentConnections(
              search,
              page
            );
        } else {
          response =
            await departmentConnectionService.getPaginatedDepartmentConnections(
              page
            );
        }
  ...
  const handleEditDepartmentConnection = useCallback(
    (id?: number) => {
      if (id) {
        const connection = departmentConnections.find(
  ...
  const handleDeleteDepartmentConnection = useCallback(
    (id?: number) => {
      if (id) {
        const connection = departmentConnections.find(
          (connection) => connection.id === id
  ...
  const handleConfirmDelete = useCallback(async () => {
    if (departmentConnectionToDelete?.id) {
      ...
      try {
        await departmentConnectionService.deleteDepartmentConnection(
          departmentConnectionToDelete.id
        );
        toast.success("Conexión eliminada", {
          description:
            "La conexión entre departamentos ha sido eliminada correctamente",
        });
  ...
  const handleSubmitDepartmentConnection = useCallback(
    async (data: DepartmentConnection) => {
      setIsLoading(true);

      try {
        if (data.id) {
          await departmentConnectionService.updateDepartmentConnection(
            data.id,
            data
          );
          toast.success("Conexión actualizada", {
            description:
              "La conexión entre departamentos ha sido actualizada correctamente",
          });
          invalidateCache();
          fetchDepartmentConnections(currentPage, searchTerm, true);
          setIsModalOpen(false);
          ...
          toast.success("Conexión creada", {
            description:
              "La conexión entre departamentos ha sido creada correctamente",
          });
  ...
  const handleDeleteMultipleDepartmentConnections = useCallback(
    async (ids: number[]) => {
      ...
      try {
        const result =
          await departmentConnectionService.deleteMultipleDepartmentConnections(
            ids
          );
        toast.success("Conexiones eliminadas", {
          description:
            result.message ||
            "Las conexiones entre departamentos han sido eliminadas correctamente",
        });
        ...
        let errorMessage =
          "Error al eliminar las conexiones entre departamentos";
  ...
  const handleExportToExcel = useCallback(async (ids: number[]) => {
    ...
    try {
      const excelBlob =
        await departmentConnectionService.exportDepartmentConnectionsToExcel(
          ids
        );
      ...
      const date = new Date();
      const fileName = `conexiones_departamentos_${date.getDate()}${
        date.getMonth() + 1
      ...
      toast.success("Exportación completada", {
        description:
          "Las conexiones entre departamentos han sido exportadas a Excel correctamente",
      });
    } catch (err) {
