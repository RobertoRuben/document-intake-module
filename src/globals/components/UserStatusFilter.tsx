    ...
    <div className="w-full">
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
        <DropdownMenuTrigger asChild>
          <Button 
            variant="outline" 
            className={`w-full font-medium ${isMobileView ? 'justify-center' : 'justify-between'}`}
          >
            <div className={`flex items-center gap-2 ${isMobileView ? 'justify-center' : ''}`}>
        ...
        <DropdownMenuContent align="end" className="w-[200px]">
          <DropdownMenuItem 
            className={`cursor-pointer ${value === 'active' ? 'bg-green-50 text-green-800' : ''}`}
            onClick={() => handleSelect('active')}
          >
            <span className="font-medium">Activos</span>
          </DropdownMenuItem>
          <DropdownMenuItem 
            className={`cursor-pointer ${value === 'inactive' ? 'bg-green-50 text-green-800' : ''}`}
